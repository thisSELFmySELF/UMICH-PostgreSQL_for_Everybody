...
-- where pg_typeof(body) will return the format type e.g jsonb
SELECT pg_typeof(body) FROM jtrack LIMIT 1;

-- FROM column called body, retrive 5 song titles(values) from the key called 'name'  (json dict key value pair)
...
SELECT pg_typeof(body->>'name') FROM jtrack LIMIT 1;
 -- :: int (return the query in the form of integer i.e cast into integer)
...
-- single arrow returns a jsonb
SELECT pg_typeof(body->'count') FROM jtrack LIMIT 1;
-- double arrow returns a text
SELECT pg_typeof(body->>'count') FROM jtrack LIMIT 1;
...
-- We have an operator to check if a tag is present
